...
<style>.datepicker-plot-area{font-family:'Vazirmatn',Tahoma,sans-serif!important;z-index:99999!important}.jalali-datetime-picker{cursor:pointer}</style>
<div class="mx-auto max-w-7xl space-y-6" dir="rtl">
    ...
    <form method="POST" action="{{ route('driver-announcements.store') }}" class="space-y-5 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        @csrf
        <div class="rounded-2xl border border-rose-200 bg-rose-50 p-5"><h1 class="text-xl font-black text-rose-900">ارسال پیام فوری به رانندگان</h1><p class="mt-2 text-sm font-semibold leading-7 text-rose-700">پس از انتشار، Push برای گوشی‌های فعال ارسال می‌شود و متن کامل هنگام بازشدن اپ نمایش داده خواهد شد. اطلاعیه اجباری تا تأیید راننده مانع ورود به داشبورد است.</p></div>
        <div class="grid gap-4 md:grid-cols-2">
            ...
            <div><label class="mb-2 block text-xs font-black text-slate-600">شروع نمایش (شمسی)</label><input type="text" name="starts_at_jalali" value="{{ old('starts_at_jalali') }}" placeholder="1405/04/25 14:30" inputmode="numeric" autocomplete="off" class="jalali-datetime-picker w-full rounded-xl border border-slate-300 px-4 py-3 text-center" dir="ltr"></div>
            <div><label class="mb-2 block text-xs font-black text-slate-600">پایان اعتبار (شمسی)</label><input type="text" name="ends_at_jalali" value="{{ old('ends_at_jalali') }}" placeholder="1405/04/26 14:30" inputmode="numeric" autocomplete="off" class="jalali-datetime-picker w-full rounded-xl border border-slate-300 px-4 py-3 text-center" dir="ltr"></div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    $('.jalali-datetime-picker').pDatepicker({
        format: 'YYYY/MM/DD HH:mm', initialValue: false, autoClose: true, responsive: true,
        calendarType: 'persian', onlySelectOnDate: false,
        calendar: { persian: { locale: 'fa', showHint: false, leapYearMode: 'algorithmic' }, gregorian: { showHint: false } },
