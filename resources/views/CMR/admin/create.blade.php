...
if(!editing){try{const recovery=JSON.parse(localStorage.getItem(draftKey)||'null');if(recovery?.fields&&confirm('یک نسخه ذخیره‌شده خودکار از فرم پیدا شد. بازیابی شود؟')){const goodsIndexes=Object.keys(recovery.fields).map(key=>key.match(/^goods\[(\d+)\]/)?.[1]).filter(Boolean).map(Number);const maxGoods=Math.max(0,...goodsIndexes);while(goodsList.children.length<=maxGoods)document.getElementById('add-good').click();Object.entries(recovery.fields).forEach(([name,value])=>{const values=[].concat(value),controls=[...form.querySelectorAll(`[name="${CSS.escape(name)}"]`)];controls.forEach((control,index)=>{const restored=values[Math.min(index,values.length-1)];if(control.type==='checkbox'||control.type==='radio')control.checked=values.includes(control.value);else control.value=restored??''})});filterMasterData()}}catch(error){localStorage.removeItem(draftKey)}}
