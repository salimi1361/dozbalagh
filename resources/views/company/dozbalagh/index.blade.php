                ...
                <tbody class="text-slate-700 text-sm divide-y divide-slate-100">
                    @forelse($requests as $item)
                        @php
                            ...
                            $displayCode = $requestType === 'renewal' && !empty($item->previous_d_code)
                                ? $item->previous_d_code
                                : $item->d_code;
                        ...
                        <tr class="hover:bg-slate-50/70 transition-colors">
                            <td class="p-4 font-mono font-bold text-slate-900">
                                <span onclick="copyToClipboard('{{ $displayCode }}', this)" class="cursor-pointer bg-slate-50 hover:bg-indigo-50 text-slate-700 hover:text-indigo-600 px-2.5 py-1.5 rounded-lg border border-slate-200 hover:border-indigo-200 transition-all inline-flex items-center gap-1.5 group" title="کلیک جهت کپی کد پرونده">
                                    {{ $displayCode }}
                                ...
                                @if($requestType === 'renewal' && $item->d_code !== $displayCode)
                                    <span class="mt-1 block text-[10px] font-sans text-slate-400">
                                        شناسه داخلی تمدید: <span class="font-mono">{{ $item->d_code }}</span>
                                    </span>
                                @endif
                            </td>
