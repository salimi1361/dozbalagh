...
<div dir="rtl" class="p-6">
    ...
    @forelse($layouts as $layout)<tr class="border-t"><td class="p-4 font-bold">{{ $layout->name }}</td><td>{{ $layout->country->name }}</td><td>{{ $layout->permit_type }}</td><td dir="ltr">{{ $layout->paper_width_mm }} × {{ $layout->paper_height_mm }} mm</td><td>{{ $layout->version }} {{ $layout->is_active ? '— فعال' : '— غیرفعال' }}</td><td><a class="text-sky-600 font-bold" href="{{ route('association.print-layouts.edit', $layout) }}">ویرایش و جانمایی</a></td></tr>
