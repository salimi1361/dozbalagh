...
@if(!$section['locked'] && $url)<a href="{{ $url }}" class="mt-4 inline-block font-bold text-emerald-700">{{ $section['company_read_only'] ? 'مشاهده وضعیت' : 'ورود به بخش' }}</a>@elseif(!$section['locked'])<span class="mt-4 inline-block text-xs font-bold text-slate-400">فرم این بخش در فاز بعد فعال می‌شود.</span>@endif
</article>
@endforeach
</section>
