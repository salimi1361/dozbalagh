...
@foreach($sections as $section)<article class="rounded-2xl border p-5 shadow-sm {{ $section['locked'] ? 'border-slate-200 bg-slate-100 opacity-70' : ($section['complete'] ? 'border-emerald-200 bg-emerald-50' : 'border-amber-200 bg-white') }}"><div class="flex items-center justify-between"><h2 class="font-black">{{ $loop->iteration }}. {{ $section['label'] }}</h2><span class="rounded-full px-3 py-1 text-xs font-bold {{ $section['complete'] ? 'bg-emerald-200 text-emerald-900' : ($section['locked'] ? 'bg-slate-200 text-slate-600' : 'bg-amber-100 text-amber-900') }}">{{ $section['complete'] ? 'تکمیل شده' : ($section['locked'] ? 'قفل' : 'نیازمند تکمیل') }}</span></div><p class="mt-3 text-sm text-slate-600">{{ $section['required'] ? 'این بخش برای ادامه فرایند اجباری است.' : 'این بخش اختیاری یا صرفاً نمایشی است.' }}</p>@if(!$section['locked'] && $section['key']==='profile')<a href="{{ route('company.shahbaz.profile.edit') }}" class="mt-4 inline-block font-bold text-emerald-700">ورود به بخش</a>@elseif(!$section['locked'] && $section['key']==='requests')<a href="{{ route('company.shahbaz.requests.index') }}" class="mt-4 inline-block font-bold text-emerald-700">ورود به بخش</a>@elseif(!$section['locked'] && in_array($section['key'],['personnel','board','shareholders']))<a href="{{ route('company.shahbaz.people.index',$section['key']) }}" class="mt-4 inline-block font-bold text-emerald-700">ورود به بخش</a>@elseif(!$section['locked'] && $section['key']==='fleet')<a href="{{ route('company.shahbaz.fleet.index') }}" class="mt-4 inline-block font-bold text-emerald-700">ورود به بخش</a>@elseif(!$section['locked'])<span class="mt-4 inline-block text-xs font-bold text-slate-400">فرم این بخش در فاز بعد فعال می‌شود.</span>@endif</article>@endforeach
</section>
