...
<div dir="rtl" class="mx-auto max-w-6xl space-y-5">
    ...
    <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        @forelse($items as $item)
            <article class="rounded-2xl border bg-white p-5 shadow-sm"><div class="flex justify-between gap-3"><b>{{ $item->person->first_name }} {{ $item->person->last_name }}</b><span class="rounded-full px-2 py-1 text-xs {{ $item->status==='archived'?'bg-violet-100 text-violet-800':'bg-emerald-100 text-emerald-800' }}">{{ $item->status==='archived'?'بایگانی':'فعال' }}</span></div><div class="mt-4 space-y-2 text-sm"><div>کد ملی/گذرنامه: {{ $item->person->national_code ?: $item->person->passport_number }}</div><div>عنوان/سمت: <b>{{ $item->job_title ?: ($item->board_position ?: ($item->shareholder_type ?: '---')) }}</b></div>@if($type==='shareholders')<div>نوع سهام: <b>{{ $item->share_type }}</b></div><div>مبلغ سهام: <b>{{ number_format((float)$item->share_amount) }} ریال</b></div><div>درصد سهام: <b>{{ rtrim(rtrim(number_format((float)$item->share_percentage,4,'.',''),'0'),'.') }}٪</b></div>@endif<div>شروع: {{ $item->started_on?verta($item->started_on)->format('Y/m/d'):'---' }}</div><div>پایان: {{ $item->ended_on?verta($item->ended_on)->format('Y/m/d'):'---' }}</div>@if($item->status==='archived' && $item->note)<div class="rounded-lg bg-violet-50 p-2 text-violet-800">دلیل بایگانی: {{ $item->note }}</div>@endif</div>
