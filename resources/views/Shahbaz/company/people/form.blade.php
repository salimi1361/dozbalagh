...
<section class="rounded-3xl border bg-white p-6">
    ...
    <div class="mt-5 grid gap-4 md:grid-cols-3">
        ...
        <label class="space-y-2"><span class="font-bold">تاریخ تولد (شمسی)</span><input type="text" readonly autocomplete="off" name="birth_date_jalali" value="{{ old('birth_date_jalali',$person->birth_date?verta($person->birth_date)->format('Y/m/d'):'') }}" class="jalali-picker w-full rounded-xl border p-3 text-center" dir="ltr" placeholder="انتخاب تاریخ"></label>
        <label class="space-y-2"><span class="font-bold">تاریخ صدور (شمسی)</span><input type="text" readonly autocomplete="off" name="issued_on_jalali" value="{{ old('issued_on_jalali',$person->issued_on?verta($person->issued_on)->format('Y/m/d'):'') }}" class="jalali-picker w-full rounded-xl border p-3 text-center" dir="ltr" placeholder="انتخاب تاریخ"></label>
...
<section class="rounded-3xl border bg-white p-6">
    ...
    <div class="mt-5 grid gap-4 md:grid-cols-3">
        ...
        @endif
        <label class="space-y-2"><span class="font-bold">تاریخ شروع همکاری (شمسی)</span><input type="text" readonly autocomplete="off" name="started_on_jalali" value="{{ old('started_on_jalali',$item->started_on?verta($item->started_on)->format('Y/m/d'):'') }}" class="jalali-picker w-full rounded-xl border p-3 text-center" dir="ltr" placeholder="انتخاب تاریخ"></label>
        <label class="space-y-2"><span class="font-bold">تاریخ پایان همکاری (شمسی)</span><input type="text" readonly autocomplete="off" name="ended_on_jalali" value="{{ old('ended_on_jalali',$item->ended_on?verta($item->ended_on)->format('Y/m/d'):'') }}" class="jalali-picker w-full rounded-xl border p-3 text-center" dir="ltr" placeholder="انتخاب تاریخ"></label>
    </div>
</section>
...
<script>document.addEventListener('DOMContentLoaded',()=>{if(window.jQuery&&typeof jQuery.fn.pDatepicker==='function'){jQuery('.jalali-picker').pDatepicker({format:'YYYY/MM/DD',initialValue:true,initialValueType:'persian',autoClose:true,responsive:true,onlySelectOnDate:true,calendarType:'persian',calendar:{persian:{locale:'fa',showHint:false,leapYearMode:'algorithmic'},gregorian:{showHint:false}},navigator:{enabled:true,scroll:{enabled:true}},toolbox:{enabled:true,calendarSwitch:{enabled:false},todayButton:{enabled:true},submitButton:{enabled:false}},timePicker:{enabled:false}});const closeOnOutsideMove=e=>{if(e.target instanceof Element&&e.target.closest('.datepicker-container,.datepicker-plot-area'))return;jQuery('.datepicker-container,.datepicker-plot-area').stop(true,true).hide();};window.addEventListener('wheel',closeOnOutsideMove,{passive:true,capture:true});window.addEventListener('touchmove',closeOnOutsideMove,{passive:true,capture:true});}@if($type==='personnel')const select=document.getElementById('job_title_choice'),wrap=document.getElementById('custom_job_title_wrap'),input=wrap.querySelector('input');const sync=()=>{const other=select.value==='__other__';wrap.classList.toggle('hidden',!other);input.required=other;if(!other)input.value='';};select.addEventListener('change',sync);sync();@endif});</script>
@endsection
