    ...
    @if($editable)
        <form method="POST" action="{{ route('company.shahbaz.branches.store') }}" class="rounded-3xl border bg-white p-6 shadow-sm">@csrf
            ...
            <div class="mt-5 grid gap-4 md:grid-cols-3">
                ...
                <label class="space-y-2"><span class="font-bold">استان</span><select id="branch_province" name="province" required class="w-full rounded-xl border p-3"><option value="">انتخاب استان</option>@foreach(array_keys($iranLocations) as $province)<option value="{{ $province }}" @selected(old('province')===$province)>{{ $province }}</option>@endforeach</select></label>
                <label class="space-y-2"><span class="font-bold">شهر</span><select id="branch_city" name="city" required data-selected="{{ old('city') }}" class="w-full rounded-xl border p-3"><option value="">ابتدا استان را انتخاب کنید</option></select></label>
