...
@forelse($companies as $company)<tr class="border-b"><td class="p-3 font-bold">{{ $company->name_fa }}</td><td>{{ $company->national_id }}</td><td><span class="rounded-full bg-slate-100 px-3 py-1 text-xs font-bold">{{ $statusLabels[$company->shahbaz_verification_status] ?? $company->shahbaz_verification_status }}</span></td><td><a class="font-bold text-blue-700" href="{{ route('association.shahbaz.companies.dossier',$company) }}">ورود به پرونده</a></td></tr>@empty<tr><td colspan="4" class="p-8 text-center">پرونده‌ای در صف نیست.</td></tr>@endforelse
