...
<head>
    ...
    <style>
        ...
        @media (min-width: 768px) {
            ...
            #sidebar:not(.sidebar-open) .h-40 { width: 5rem; min-width: 5rem; height: 6rem; padding: 0; }
            #sidebar:not(.sidebar-open) .h-40 img { height: 3.5rem; }
...
<body class="flex h-screen overflow-hidden">
    ...
    <main class="flex-1 flex flex-col min-w-0 overflow-hidden relative">
        ...
        <header class="h-20 bg-white/80 backdrop-blur-md border-b border-slate-200 flex items-center justify-between gap-4 px-4 sm:px-6 lg:px-8 z-30 sticky top-0">
            <div class="flex min-w-0 items-center gap-4">
                <button onclick="toggleSidebar()" aria-label="باز و بسته کردن منو" aria-controls="sidebar" aria-expanded="true" class="shrink-0 p-2 text-slate-500 hover:bg-slate-100 rounded-lg">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"></path></svg>
                </button>

                <h1 class="min-w-0 truncate text-sm sm:text-base font-black text-slate-800">@yield('header_title', 'سامانه مدیریت دوزبلاغ')</h1>
            </div>
